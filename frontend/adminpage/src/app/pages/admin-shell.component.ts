...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, RouterOutlet, RouterLink, RouterLinkActive, MobileDrawerMenuComponent, LiquidGlassDirective],
...
export class AdminShellComponent {
  ...
  readonly publicStoreUrl = computed(() => {
    const slug = this.currentStore()?.slug?.trim().replace(/^\/+|\/+$/g, '');

    if (!slug) {
      return null;
    }

    return `https://bienhelodias.qzz.io/${encodeURIComponent(slug)}`;
  });
